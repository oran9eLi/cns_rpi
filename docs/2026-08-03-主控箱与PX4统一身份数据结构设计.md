...
PX4 同样使用 `OPEN_DRONE_ID_BASIC_ID.uas_id` 作为 `device_id`，但这条消息不在
PX4 默认遥测流里，需要树莓派按 §3.2 主动请求。
...
两类设备的身份都来自 `OPEN_DRONE_ID_BASIC_ID`，但获取方式不同：主控箱固件周期
主动发送，PX4 在普通遥测链路上默认不发这条消息，必须由树莓派主动请求。
...
flowchart TD
    ...
    KIND{"device_type"}
    BOX["主控箱：固件周期发送，树莓派被动接收"]
    REQ["PX4：树莓派周期请求 OPEN_DRONE_ID_BASIC_ID"]
    ...
    WAIT["保持身份未就绪，继续等待或继续请求"]

    HB --> TYPE --> KIND
    KIND -->|cns_box| BOX --> BASIC
    KIND -->|flight_controller| REQ --> BASIC
    BASIC --> VALID
    VALID -->|是| READY --> MQTT
    VALID -->|否| WAIT
```
...
### 3.2 PX4 Basic ID 主动请求

主控箱固件按约定周期向串口发送 `OPEN_DRONE_ID_BASIC_ID`，树莓派只需被动接收。
PX4 不在默认遥测流里包含该消息，树莓派确认 `device_type` 为 `flight_controller`
后必须周期请求，直到拿到合法 `uas_id` 为止：

| 项 | 约定 |
|---|---|
| 请求方式 | `MAV_CMD_REQUEST_MESSAGE`，`param1 = 12900` |
| 目标 | 当前已锁定的 MAVLink 主端点 |
| 周期 | 2 秒，与既有 `AUTOPILOT_VERSION` 请求节拍一致 |
| 停止条件 | `device_id` 就绪 |

`AUTOPILOT_VERSION` 使用同一套请求机制，但两者的停止条件互相独立：身份请求在
`device_id` 就绪后停止，版本请求在产品与版本信息就绪后停止。不得把版本请求的
停止条件写成"`device_id` 已就绪"——统一身份后 `device_id` 来自 Basic ID，会先于
`AUTOPILOT_VERSION` 到达，那样 PX4 的产品与版本元数据将永远采集不到。

两条请求不在同一轮循环里连发，错开发送，使 `COMMAND_ACK` 的时序可以和日志对应。
`MAV_CMD_REQUEST_MESSAGE` 的应答不携带被请求的消息号，无法仅从 ACK 区分是哪条
请求被拒绝。

若真机验证发现 PX4 对 `MAV_CMD_REQUEST_MESSAGE` 不响应，改用
`MAV_CMD_SET_MESSAGE_INTERVAL`（`param1 = 12900`，`param2 = 1000000`）显式配置
1 Hz 流，其余流程不变。
...
- PX4 继续按 §3.2 的节拍请求 Basic ID；
- PX4 的 QGC 本地桥接可以继续工作；
- 不创建按设备寻址的 MQTT Client；
- 不发布注册、遥测或命令 ACK；
- 日志按限频策略提示身份未就绪。

限频日志必须区分两种原因，否则现场无法判断是固件未配置身份还是链路异常：

| 现象 | 日志语义 |
|---|---|
| 已按节拍请求但始终没收到 Basic ID | PX4 未响应身份请求，可能未配置 UAS ID |
| 收到 Basic ID 但校验不通过 | `uas_id` 非法，附带实际取到的值 |

系统不使用 PX4 `uid/uid2`、树莓派序列号或 MAVLink `sysid` 生成临时主键。PX4 侧
未配置 UAS ID 时，该设备不会上线，这是 §8.3 已确认的取舍，不做降级处理。
...
`AUTOPILOT_VERSION` 按 §3.2 的机制请求，停止条件是产品与版本信息就绪，与身份
请求相互独立。未收到 `AUTOPILOT_VERSION` 不阻止设备建立 MQTT 会话。收到或更新
版本信息后，树莓派重新发布 retained online registration，使服务器幂等更新元数据。
...
主控箱和 PX4 都使用 Basic ID 中的 `uas_id` 寻址。旧
`{device_id}/telemetry` 与 PX4 专用 `{device_id}/px4/realtime/v1` 已废弃，设备不
双发；完整契约见 `高低频遥测发布协议.md`。

### 5.2 协议版本

本次变更会删除字段并改变 PX4 `device_id` 语义，属于不兼容变更。注册与常规遥测的
`schema_version` 从 `2` 升级为 `3`。通用实时遥测和 PX4 RTT 探测各自使用独立
`schema_version=1`，其载荷中的 `device_id` 同样来自 Basic ID。
...
- 通用 MAVLink 遥测解码；
- 主控箱私有遥测与命令；
- PX4 官方命令、QGC 桥接和 RTT 探测；
- 主控箱与 PX4 共用的快照/实时遥测通道；
- registration retained online/offline 机制。

### 8.2 本次需要同步修改

- 树莓派身份解析、状态存储和 MQTT 建连条件；
- PX4 身份与版本消息的主动请求时序（两条请求的停止条件必须分开）；
- 注册、快照遥测、实时遥测和 RTT 载荷中的 `device_id`；
- JSON schema 及单元测试；
...
1. 主控箱和 PX4 都只有收到合法 `OPEN_DRONE_ID_BASIC_ID.uas_id` 后才建立设备
   MQTT 会话；
2. 确认 `device_type` 为 `flight_controller` 后，树莓派按固定节拍主动请求
   `OPEN_DRONE_ID_BASIC_ID`，直到身份就绪才停止请求；
3. PX4 先收到 Basic ID、后收到 `AUTOPILOT_VERSION` 的时序下，产品与版本元数据
   仍能被采集到并进入注册（验证两条请求的停止条件确实互相独立）；
4. PX4 始终不响应身份请求时，限频日志能指出是"未响应"还是"`uas_id` 非法"；
5. 两类设备的 MQTT topic、注册、遥测和命令路由均使用同一个 `device_id`；
6. 对外 JSON 不再以字段键形式出现 `vendor_id`、`remote_id`、`uid`、`uid2`、
   `gateway_id`、`rpi_serial` 或 `endpoint`；能力列表中的 `remote_id` 仍表示设备
   支持 Remote ID 功能；
7. `drone_id.basic_id` 不重复输出 `uas_id`；
8. PX4 未收到 `AUTOPILOT_VERSION` 时仍可正常注册和上传遥测；
9. 收到 `AUTOPILOT_VERSION` 后只更新产品与版本元数据，不改变 `device_id`；
10. 树莓派更换或重装不改变受控设备身份；
11. 串口换接设备或身份冲突时旧设备正确离线，新身份重新建连；
12. 服务器能够以一个 `device_id` 完成注册 upsert、在线状态、遥测归档和命令路由；
13. 主控箱现有 `DCDWCNS1...` topic 字符串保持不变。
