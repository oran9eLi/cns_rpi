...
> 身份模型更新：本文关于 PX4 使用 `AUTOPILOT_VERSION.uid2/uid` 作为
> `device_id`、树莓派使用 `gateway_id`、注册/遥测 schema v2 身份字段及相应
> 数据库结构的内容，已由
> `docs/2026-08-03-主控箱与PX4统一身份数据结构设计.md` 取代。设备识别、遥测兼容、
> QGC 桥接和命令适配等其余内容继续有效。本文第 9 节的遥测 Topic 与 QoS 表也已由
> `docs/高低频遥测发布协议.md` 取代，不得再作为现行服务端订阅依据。
