...
- 已占用时不支持强制踢掉当前控制端，避免飞行中控制权突然切换。
- 当前 QGC 正常关闭或断网后，最迟约 10 秒释放；随后另一台会自动获得控制权。
...
```text
QGC UDP单控制权桥接已启动: listen=14540 qgc=14550 interfaces=wlan0,wg0 policy=first_valid_wins commands=enabled
```
