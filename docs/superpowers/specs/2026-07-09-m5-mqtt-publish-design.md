...
**网络路径注意事项**：如果真机上5G模块（Quectel RM500U-CNV）还插着且已拨号，`usb0`的默认路由metric（100）比`wlan0`（600）低，5G实际上比WiFi优先级更高——想确保这次验证走WiFi，要么把5G模块拔了/不让它拨号，要么手动调整路由metric。这是运维/网络层面的注意事项，不是`mqtt/`模块代码需要处理的问题。5G用的还不是最终物联卡（当前测试用手机卡），真实联网切换测试留到M8。
