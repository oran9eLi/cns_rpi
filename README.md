...
阶段一 M1 的 Pi 端新增 `set_f407_baud`、`uart_probe`、`set_pi_baud`：仅在已核验主控箱身份和可信串口拓扑下，先落盘动作身份再按类型 3/4 记录旧速率接受事实；失配探测沿用 H1 Echo，三轮各有独立随机数和真实帧；Pi 本地换速先保存 `serial.baud`，再由同一进程关闭、重开并读回串口。F407 方报告已更新固件，但 Pi 端尚未部署和完成 57600 实机联调；离线单元测试不能证明 F407 的实际换速行为。约定与恢复边界见 [M1 树莓派端实施记录](docs/change_history/2026-09-24_M1树莓派端.md)。
