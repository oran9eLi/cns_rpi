...
阶段一 H1 的 `uart_echo` 已完成树莓派端候选实现：沿用唯一串口读写者发送 MAVLink `TUNNEL` 请求，核对 F407 回显、双方实际波特率和动作后新业务帧，再向 `experiment/ack` 发布完整帧与单调 RTT。2026-09-23 已在实体 Pi 上通过仅监听本机环回的隔离 Broker，实测英文与中英混合文本原样回显，双方报告速率均为 115200。生产服务未替换；现网 Broker 来源门禁与 Server/Web 闭环未验收，仍不得将 H1 接入现网。详见 [H1 本端验证与待验收记录](docs/阶段一H1真实回显树莓派端离线实现与待验收.md)。
