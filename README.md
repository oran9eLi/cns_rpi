...
发布收尾状态见 [发布准备清单](docs/V1.0发布准备清单.md)，V1.0 基线能力与验收边界见 [V1.0.0 发布说明](docs/V1.0发布说明.md)，当前修订内容见 [V1.0.2 发布说明](docs/V1.0.2发布说明.md)。
...
阶段一 H1 的 `uart_echo` 已完成树莓派端候选实现：沿用唯一串口读写者发送 MAVLink `TUNNEL` 请求，核对 F407 回显、双方实际波特率和动作后新业务帧，再向 `experiment/ack` 发布完整帧与单调 RTT。2026-09-23 已在实体 Pi 完成隔离 Broker 测试，并将提交 `0b86a59` 部署到生产服务；现网直达 MQTT 的受限文本回显成功，双方报告速率均为 115200。Broker 匿名来源控制与 Server/Web 闭环仍未验收，不将 Pi↔F407 单项通过写成跨端完成。详见 [H1 本端验证记录](docs/阶段一H1真实回显树莓派端离线实现与待验收.md)和[现网部署记录](docs/2026-09-23-H1树莓派现网部署记录.md)。

阶段一 M1 的 Pi 端新增 `set_f407_baud`、`uart_probe`、`set_pi_baud`：仅在已核验主控箱身份和可信串口拓扑下，先落盘动作身份再按类型 3/4 记录旧速率接受事实；失配探测沿用 H1 Echo，三轮各有独立随机数和真实帧；Pi 本地换速先保存 `serial.baud`，再由同一进程关闭、重开并读回串口。2026-09-24 已在实体 Pi 取得 B0/B1 两档真实业务帧与 Echo、F407 实际回报 57600，以及实体复位后恢复 B0 的证据；Server/Web 闭环仍待验收。详见 [M1 树莓派端实施记录](docs/change_history/2026-09-24_M1树莓派端.md)。
