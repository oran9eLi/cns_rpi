...
阶段一 H1 的 `uart_echo` 已完成树莓派端**离线候选实现**：在同一 `experiment/set` 接收受限 UTF-8 文本，经现有唯一串口读写者发送候选 MAVLink `TUNNEL` 请求，核对 F407 回显、实际波特率和本动作后新业务帧，再向 `experiment/ack` 发布完整帧与单调 RTT 事实。发帧前将动作身份写入持久化去重记录；重启后未决动作只返回 `result_uncertain`，不自动重发。此能力未部署、未做真实 F407 验收，现场 B0、接线、F407 固件及三端字节契约仍需共同冻结；现阶段不得向实体设备派发 H1。详见 [H1 离线实现与待验收记录](docs/阶段一H1真实回显树莓派端离线实现与待验收.md)。
